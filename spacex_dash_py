...
def scatterplot_payload_success(entered_site, payload):
    min = payload[0]
    max = payload[1]
    filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].between(min,max)]
    if(entered_site == 'ALL'):
        ...
        #data_launchsite = data['Payload Mass (kg)']
        #print(data_launchsite)
        fig = px.scatter(data, x = "Payload Mass (kg)", y = "class", color = "Booster Version Category")
        return fig
